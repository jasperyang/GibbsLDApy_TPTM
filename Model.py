...
from TPTM import *
import sys   
sys.setrecursionlimit(1000000) #迭代深度:这里设置为一百万
...
class Model(object):
    ...
    def init(self, argc, argv):
        ...
        self.ut = Utils()
        self.ut.process_source('data/1.xml',self.split_num)
            ...
            for i in range(self.twords) :
                ...
                for key in self.id2word.keys() :
                    if list(words_probs[i].keys())[0] == int(key) :
                        found = True
                        word = self.id2word[key]
                        break
                if found :
                    # tmp = "\t" + str(list(words_probs[i].keys())[0]) + " " + str(list(words_probs[i].values())[0]) + '\n'
                    tmp = "\t" + str(word) + " " + str(list(words_probs[i].values())[0]) + '\n'
                    fout.write(tmp)
    ...
    '''
    # @tested
    def save_final_prediction(self, filename):

        newtheta = np.zeros((self.tp.split_num, self.tp.total_topic))
        start = 0
        count = 0
        for shot in range(self.tp.split_num):
            length = len(self.tp.shots[shot])
            for topic in range(self.tp.total_topic):
                for i in range(length):
                    newtheta[count][topic] += self.theta[start + i][topic]
        ...
        self.p_w_s = np.dot(newtheta,self.phi)
        
        np.save('data/progress/p_w_s.npy', self.p_w_s)
        
        for num in range(self.tp.split_num):
            ...
            for iteration in range(K):
                ...
                for word_pro in a:
                    ...
                    count += 1
                a[location] = 0
                keep.append({location:max_})
            
            tmp = "shot " + str(num) + "th:\n"
            fout.write(tmp)
            for i in range(self.twords):
                found = False
                keep_key = keep[i].keys()[0]
                for key in self.id2word.keys():
                    if  keep_key == int(key):
                        found = True
                        word = self.id2word[key]
                ...
                if found:
                    # tmp = "\t" + str(list(words_probs[i].keys())[0]) + " " + str(list(words_probs[i].values())[0]) + '\n'
                    tmp = "\t" + str(word) + " " + str(keep[i].values()[0]) + '\n'
                    fout.write(tmp)
            ...
            for i in range(self.twords):
                ...
                if not found2 :
                    ...
                    if found :
                        ...
                        tmp = "\t" + word + " " + list(words_probs[i].values())[0] + '\n'
                        fout.write(tmp)
    ...
    def estimate(self):
        ...
        self.tp = TPTM(self.split_num,self.K,10000,self.nw,self.ut,self.z) # 这里设置默认参数，后期封装到外层
        self.tp.preprocessing()
        self.alpha_c = self.tp.alpha_c
        ...
        for self.liter in range(last_iter+1,self.niters+last_iter) :
            ...
            if (self.liter%200)==0 and ((self.liter/200)%2) == 0: # 偶数更新 x_u_c_t 值 
                print('x_u_c_t')
                self.tp.update_x_u_c_t(self.liter)
                self.tp.update_Mpre_c(self.liter)
                self.tp.update_Mpre_s(self.liter)  
                start = 0
                # 先更新 pi_c 再去更新　alpha_c
                for q in range(self.tp.split_num):
                    for i in range(start, start + len(self.tp.com[q])):
                        for topic in range(self.K):
                            self.tp.pi_c[i][topic] = (self.tp.lambda_s[q][topic]*self.tp.comment_all.ix[i, 2+self.K+2+self.K+topic]+self.tp.M_pre_c[i][topic])
                    start+=len(self.tp.com[q])
                self.alpha_c = self.tp.Get_alpha_c()
            elif (self.liter%200)==0 and ((self.liter/200)%2) != 0: # 奇数更新 lambda 值
                print('lambda_s')
    ...
    def compute_theta(self):
        for m in range(self.M) :
            ...
            for k in range(self.K):
                Kalpha += self.alpha_c[m][k]
            for k in range(self.K) :
                self.theta[m][k] = (self.nd[m][k] + self.alpha_c[m][k]) / (self.ndsum[m] + Kalpha)
    ...
    def inference(self):
        ...
        self.tp.load_stage_data()
        self.alpha_c = self.tp.Get_alpha_c()
        self.liter = self.tp.iteration


        if self.liter != 0 and (self.liter%50)==0: 
            self.tp.update_Mpre_c(self.liter)
            self.tp.update_Mpre_s(self.liter)
        elif self.liter != 0 and (self.liter%2)==0: # 偶数更新 x_u_c_t 值
            self.tp.update_x_u_c_t(self.liter)
        elif self.liter != 0 and (self.liter%2) != 0: # 奇数更新 lambda 值
            self.tp.update_lambda_s(self.liter)
                
        start = 0
        # 先更新 pi_c 再去更新　alpha_c
        if len(self.tp.comment_all.columns)>34 :
            for q in range(self.tp.split_num):
                for i in range(start, start + len(self.tp.com[q])):
                    for topic in range(self.K):
                        self.tp.pi_c[i][topic] = (self.tp.lambda_s[q][topic] * self.tp.comment_all.ix[i, topic + 34] + self.tp.M_pre_c[i][topic])/1000.0
                start+=len(self.tp.com[q])
        self.alpha_c = self.tp.Get_alpha_c()
